...
from scripts.ml.models import UnifiedModel
...
def train_fold(fold_data, args, device, plot_dir):
    ...
    val_ds = ManifestDataset(fold_data["val"]["pos"], fold_data["val"]["neg"], desc=f"Fold {fold_idx} Val")
...
def main():
    ...
    parser.add_argument("--manifest", type=str, default=os.path.join("d:\\", "Project", "Research", "experiments", "apEX_auth_folds.json"))
    ...
    for fold_data in manifest["folds"]:
        res = train_fold(fold_data, args, device, plot_dir)
        results.append(res)
    ...
    plt.title(f"10-Fold Cross Validation Summary - {manifest['target_name']}")
